        ...
        {slides.map((slide, index) => (
          <SwiperSlide key={index} className="relative">
            ...
            <div className={`slider-text absolute ${theme ? 'bg-white/70' : 'bg-black/60'} bottom-0 w-full shadow-xl rounded-b-2xl flex flex-col`}>
              ...
              <a href={slide.link} className="cursor-pointer"><button className={`view-btn text-sm w-[50%] md:w-[30%] rounded-md font-semibold bg-zinc-800 text-white`}>View Live</button></a>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
